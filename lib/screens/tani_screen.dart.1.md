...
// ──────────────────────────────────────────────────────────────────────────────
// File de signes : on avance au signe suivant UNIQUEMENT quand success == false
// (c.-à-d. le back n'a plus de prédictions disponibles pour ce signe).
// Le nombre d'appels par signe est entièrement piloté par le serveur.
// ──────────────────────────────────────────────────────────────────────────────
class _SignQueue {
  final _rnd = Random();
  ...
  /// À appeler uniquement quand le back renvoie success == false pour [current].
  void advance() {
    _queueIndex++;
    if (_queueIndex >= _queue.length) {
      ...
      _queue = List<String>.from(kZodiacSigns);
      _queue.shuffle(_rnd);
      // Éviter la même enseigne deux fois de suite en bout de liste
    ...
    debugPrint('⏭️  Signe épuisé → passage au suivant : ${current}');
  }
}
...
class _TaniScreenState extends State<TaniScreen> with TickerProviderStateMixin {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // Logique principale : on garde le signe courant tant que success == true.
  // Si success == false → le back n'a plus de prédictions pour ce signe,
  // on appelle advance() pour passer au suivant, puis on relance immédiatement.
  // ────────────────────────────────────────────────────────────────────────────
  Future<void> _fetchAndPlay() async {
    ...
    // Boucle de rotation : on cherche un signe qui a encore des prédictions.
    // On s'arrête après avoir parcouru tous les signes (sécurité anti-boucle infinie).
    int attempts = 0;
    final maxAttempts = kZodiacSigns.length;

    while (attempts < maxAttempts) {
      final sign = _signQueue.current;
      debugPrint('🔮 Tentative avec le signe : $sign (essai ${attempts + 1}/$maxAttempts)');
      ...
      if (prediction == null) {
        // Erreur réseau / serveur → fallback TTS sans changer de signe
        debugPrint('❌ prediction == null (erreur réseau) pour $sign');
        break;
      }

      if (prediction.success) {
        // ✅ Prédiction valide → on l'utilise, on NE change PAS de signe
        debugPrint('✅ success=true pour $sign → prédiction utilisée');
        break;
      } else {
        // ⏭️  success=false → ce signe est épuisé, on passe au suivant
        debugPrint('⏭️  success=false pour $sign → avance au signe suivant');
        _signQueue.advance();
        attempts++;
        prediction = null; // on va retenter
      }
    }
    ...
    if (prediction == null || !prediction.success) {
      debugPrint('❌ Aucune prédiction disponible → TTS fallback');
      _speakFallback();
      return;
    }
